...
export function UsageSection({ rows }: { rows?: UsageRow[] }) {
  ...
  const data = useMemo(() => (rows && rows.length > 0 ? rows : genMockUsage(30)), [rows])

  const totals = tokensOf(data)
  const totalCost = data.reduce((a, r) => a + estimateCost(r).total, 0)
  const daily = dailyAggregate(data)
  const models = modelAggregate(data)
  const maxDaily = Math.max(1, ...daily.map((d) => d[1] + d[2]))
  const modelTotal = Math.max(1, models.reduce((a, m) => a + m.input + m.output, 0))
  ...
  const recent = [...data].sort((a, b) => (a.ts < b.ts ? 1 : -1)).slice(0, 8)
