...
**统计口径**:TEST 模式下**未开 MOCK**时,站长本人的访问仍被排除(且 `isTestMode()` 本身就要求站长),
所以 TEST 里 PV/UV 天然为 0;**开启 MOCK 后放行埋点**,按该 mock cid 写入**测试库**,
便于验收多身份 PV/UV/项目点击(见 `apps/next-home/src/app/api/track/route.ts` 与 `docs/STATS.md`)。
